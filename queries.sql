...
joins_exercise-# GROUP BY (first_name, last_name) ORDER BY first_name;
-- Result 2:
...
-- Query 3:
joins_exercise=# SELECT first_name, last_name, ROUND(AVG(price)) as average_price, COUNT(owner_id) FROM owners o
joins_exercise-# JOIN vehicles v on o.id=v.owner_id GROUP BY (first_name, last_name) HAVING COUNT(owner_id) > 1
joins_exercise-# AND ROUND(AVG(price)) > 10000 ORDER BY first_name DESC;
-- Result 3:
